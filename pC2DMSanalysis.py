...
from pC2DMSUtilsModule import readTextFile, maxIndices, varII, covXI, cutAC, saveSyxEinSum, scaleToPower, circList, clearCirc, bsResamps
...
class Map:
    ...
    def plot(self, mapTitle='', save=False, figFileName='', \
        ...
        toPlot = scaleToPower(cutAC(self.array)) #these three operations
    ...
    maxRayLength='square_width',r_i='square_centre',c_i='square_centre'):
        ...
        for theta in np.linspace(0, 2*np.pi, numRays, endpoint=False):
            ...
            for x in range(maxRayLength):
                r+=incR
                c+=incC
                if array[np.round(r), np.round(c)]<=integThresh:
...
if __name__=='__main__':
    ...
    parser.add_argument("--agctarget", type=int, required=False)
    parser.add_argument("--numscans", type=int, required=False)
    parser.add_argument("--numfeats", type=int, required=False)
    
    parser.add_argument("--clearrad", type=int, required=False)
    
    parser.add_argument("--chemfilt", type=list, required=False)
    parser.add_argument("--chemfilttol", type=float, required=False)
    
    parser.add_argument("--shapefilt", type=bool, required=False)
    parser.add_argument("--shfiltthr", type=int, required=False)
    parser.add_argument("--shfiltref", type=int, required=False)
    parser.add_argument("--shfiltrad", type=int, required=False)
    
    parser.add_argument("--breaktime", type=int, required=False)
    
    parser.add_argument("--pixout", type=int, required=False)
    parser.add_argument("--compixout", type=int, required=False)
    parser.add_argument("--cutpeaks", type=bool, required=False)
    parser.add_argument("--integthresh", type=int, required=False)
    parser.add_argument("--numrays", type=int, required=False)
    parser.add_argument("--perimoffset", type=int, required=False)
    parser.add_argument("--pixwidth", type=int, required=False)
    parser.add_argument("--sampling", type=int, required=False)
    parser.add_argument("--bsrs", type=int, required=False)
    parser.add_argument("--bsrs_diffs", type=int, required=False)
    parser.add_argument("--saveat", type=int, required=False)
    parser.add_argument("--basepath", type=int, required=False)
    parser.add_argument("--uset4r", type=int, required=False)
    parser.add_argument("--printat", type=int, required=False)
    ...
    #%% These are currently all the default values in the classes above
    AGCtarget=100 if args.agctarget is None else args.agctarget
    numScans='all' if args.numscans is None else args.numscans
    numFeats=3000 if args.numfeats is None else args.numfeats
    
    clearRad=25 if args.clearrad is None else args.clearrad
    
    chemFilt=[] if args.chemfilt is None else args.chemfilt
    chemFiltTol=2.0 if args.chemfilttol is None else args.chemfilttol
    
    shapeFilt=False if args.shapefilt is None else args.shapefilt
    shFiltThr=-0.2 if args.shfiltthr is None else args.shfiltthr
    shFiltRef='map' if args.shfiltref is None else args.shfiltref
    shFiltRad=15 if args.shfiltrad is None else args.shfiltrad
    
    breakTime=3600 if args.breaktime is None else args.breaktime
    
    pixOut=15 if args.pixout is None else args.pixout
    comPixOut=3 if args.compixout is None else args.compixout
    cutPeaks=True if args.cutpeaks is None else args.cutpeaks
    integThresh=0 if args.integthresh is None else args.integthresh
    numRays=100 if args.numrays is None else args.numrays
    perimOffset=-0.5 if args.perimoffset is None else args.perimoffset
    pixWidth=1 if args.pixwidth is None else args.pixwidth
    sampling='jackknife' if args.sampling is None else args.sampling
    bsRS=args.bsrs #because default is None for bsRS
    bsRS_diffs=args.bsrs_diffs #because default is None for bsRS_diffs
    saveAt=False if args.saveat is None else args.saveat
    basePath=args.basepath #because default is None for basePath
    useT4R=False if args.uset4r is None else args.uset4r
    printAt=50 if args.printat is None else args.printat
    #%%
    nameDeets=raw_input('file name suffix for saved feature list:\n')
    #%% Now run the analysis
    readTextFile(args.textfile, args.readto)    
    scan1=Scan(args.readto, AGCtarget=AGCtarget)
    map1=PCovMap(scan1, scan1.tic(), numScans=numScans)
    
    feats=map1.analyse(numFeats, clearRad=clearRad, chemFilt=chemFilt, \
    chemFiltTol=chemFiltTol, shapeFilt=shapeFilt, shFiltThr=shFiltThr,\
    shFiltRef=shFiltRef, shFiltRad=shFiltRad, breakTime=breakTime,\
    pixOut=pixOut, comPixOut=comPixOut, cutPeaks=cutPeaks,\
    integThresh=integThresh, numRays=numRays, perimOffset=perimOffset,\
    pixWidth=pixWidth, sampling=sampling, bsRS=bsRS, bsRS_diffs=bsRS_diffs,\
    saveAt=saveAt, basePath=basePath, useT4R=useT4R, printAt=printAt)
    
    np.save(args.readto+'/'+str(numFeats)+'feats_'+nameDeets+'.npy', feats)
    #%% Save the parameters of the feature file in a text file
    f=open(args.readto+'/'+str(numFeats)+'feats_'+nameDeets+\
    '_parameters.txt', 'w')
    
    f.write('AGC MSn target (AGCtarget): '+str(AGCtarget)+'\n')
    f.write('Number of scans (numScans): '+str(numScans)+'\n')
    f.write('Number of features (numFeats): '+str(numFeats)+'\n')
    f.write('Clear radius (clearRad): '+str(clearRad)+'\n')
    f.write('Chemical filter for (chemFilt): '+str(chemFilt)+'\n')
    f.write('Chemical filter tolerance (chemFiltTol): '+str(chemFiltTol)+'\n')
    f.write('Shape filter on (shapeFilt): '+str(shapeFilt)+'\n')
    f.write('Shape filter threshold (shFiltThr): '+str(shFiltThr)+'\n')
    f.write('Shape filter reference (shFiltRef): '+str(shFiltRef)+'\n')
    f.write('Shape filter radius (shFiltRad): '+str(shFiltRad)+'\n')
    f.write('Break time (breakTime): '+str(breakTime)+'\n')
    f.write('Pixels out (pixOut): '+str(pixOut)+'\n')
    f.write('Centre of mass pixels out (comPixOut): '+str(comPixOut)+'\n')
    f.write('Cut peaks before integration (cutPeaks): '+str(cutPeaks)+'\n')
    f.write('Integration threshold (integThresh): '+str(integThresh)+'\n')
    f.write('Number of rays (numRays): '+str(numRays)+'\n')
    f.write('Perimeter offset (perimOffset): '+str(perimOffset)+'\n')
    f.write('Pixel width (pixWidth): '+str(pixWidth)+'\n')
    f.write('Sampling (sampling): '+str(sampling)+'\n')
    #bsRS    
    #bsRS_diffs
    f.write('Save after how many (saveAt): '+str(saveAt)+'\n')
    #basePath
    f.write('Use [old, rigid] template 4 ray routine? (useT4R): '+str(useT4R)\
    +'\n')
    #printAt
    f.close()