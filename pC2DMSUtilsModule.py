...
def isInFeatList(mzs, featList, ionThresh=1.5):
...
def byPosIons(peptide_sequence):
    ...
    peptide_p = peptide_mass + 1.00782
    peptide_pp = (peptide_mass + (2 * 1.00782))/2
    peptide_ppp = (peptide_mass + (3 * 1.00782))/3
    peptide_pppp = (peptide_mass + (4 * 1.00782))/4
    
    table[0, 0] = '%.4f'%peptide_mass
    table[2, 0] = '%.4f'%peptide_p
    table[3, 0] = '%.4f'%peptide_pp
    table[4, 0] = '%.4f'%peptide_ppp
    table[5, 0] = '%.4f'%peptide_pppp
    
    return table

def idIons(peakList, (sequence, parentZ), NLs=['H2O','NH3'], fragIonTol=1.5, \
sortCol=4, printNo=50, mzDev=False, acThresh=5.5, loseAA=2):
    ...
    NLMonoisotopicMasses = {'H2O': 18.01056, 'NH3': 17.02653, 'CH3': 14.0157,'H3PO4': 97.97689, 'HPO3': 79.96633, 'SO3': 79.95682,  'None': 0.0}
    ...
    for ionPair in ionList:
        ...
        for compareCols in colMatchDict[parentZ]:
            ...
            for NL1 in NLs:
                for row1 in range(1, sequenceLength): #not sequenceLength + 1 because you can't have a b1/y(n-1) correlation
                        ...
                        for bIonAt in compareCols[1]:
                            ...
                            for NL2 in NLs:
                                for numLostAA1 in range(loseAA+1):
                                    ...
                                    if abs(ion2 - (table[row2, bIonAt] - NLMonoisotopicMasses[NL2]/bIonCharge)) <= fragIonTol and row2 > 1: #row2 > 1 because can't have b1 ions
                                        ...
                                        ion2mz = table[row2, bIonAt] - NLMonoisotopicMasses[NL2]/bIonCharge #took the rounding from here 20160427 
                                    
                            #For a ions
                            for NL2 in NLs:
                                for numLostAA2 in range(loseAA + 1):
                                    row2 = sequenceLength-row1-numLostAA2
                                    if abs(ion2 - (table[row2, bIonAt] - (NLMonoisotopicMasses[NL2] + 27.99492)/bIonCharge)) <= fragIonTol and row2 > 1: #27.99492 is the monoisotopic mass of CO. row2 > 1 because can't have b1 ions
                                        goodCorrelation = True
                                        hits += 1
                                        if NL2 == 'None':
                                        ...
                                        ion2mz = table[row2, bIonAt] - (NLMonoisotopicMasses[NL2] + 27.99492)/bIonCharge#took the rounding from here 20160427 
                            ...
                            if goodCorrelation:     
                                ...
                                ion1mz = table[row1, yIonAt] - NLMonoisotopicMasses[NL1]/yIonCharge#took the rounding from here 20160427
                                #changed20160427 (from parentZ to yIonCharge for divisor of NLMosoisotopicMass)
                                
        appendum = [(ion1Name, ion1mz), ion1, (ion2Name, ion2mz), ion2, round(ionPair[3], 2), round(ionPair[2], 2), goodCorrelation, hits]
    ...
    ionsFrom=raw_input('Please provide short description of the experimental data:\n')
    rawDataPath=raw_input('Please provide the full path of the feature list numpy array:\n')
    ...
    for x in range(printNo):
        ...
        if matchesOrdered[x][6]:
            print matchesOrdered[x][0][0], '@', round(matchesOrdered[x][1], 2), '&', matchesOrdered[x][2][0], '@', str(round(matchesOrdered[x][3], 2))+':', matchesOrdered[x][sortCol] 
            ...
            print round(matchesOrdered[x][1], 2), '&', str(round(matchesOrdered[x][3], 2))+':', matchesOrdered[x][sortCol]
            #strange formatting syntax before colon in order to have no space between colon and preceeding number
            #put the rounding here 20160427
    ...
    if mzDev:
        ...
        #mass deviation analysis
            if matchesOrdered[x][6]:
                ...
                print 'Peak ',x + 1,' - ',matchesOrdered[x][0][0],': ',round(mzDev1, 2),' & ',matchesOrdered[x][2][0],': ',round(mzDev2, 2) 
                #put the rounding here 20160427
                
        print 'Total RMS deviation of ',countForRMS,'islands identified at tol. ',fragIonTol,' (',countForRMS*2,' m/zs)'
...
def readTextFile(textFile, scanFolder):
    ...
    fullScanFolder = scanFolder #CHANGED. For personal use this had a prefix
    #because all scan folder had the same base path which I didn't want to
    #type all the time
